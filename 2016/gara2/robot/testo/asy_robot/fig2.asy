...
void drawwalk(pair start, string dirs, pen col) {
    path p = (start.x + 0.5, -start.y - 0.5) + dir(unitrand()*360)/5;
    for (int i=0; i<length(dirs); ++i) {
        ...
        p = p -- (start.x + 0.5, -start.y - 0.5) + dir(unitrand()*360)/5;
    }
    path r = roundedpath(p, .3);
    draw(r, col, EndArrow);
}

string[] T = {'RLR', 'LRL', 'RLR'};

srand(0);
drawmatrix(T);
drawwalk((-1,0), "ESESES", red);
drawwalk((2,-1), "SOSOSO", blue);
drawwalk((3,2),  "ONONON", green);
drawwalk((0,3),  "NENENE", purple);
drawwalk((-1,0), "E", grey);
