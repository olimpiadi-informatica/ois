/**
 * Dijkstra ottimizzato, meno efficiente di 0-1 BFS
 */
...
int estimate(int u, int rows, int columns) {
    // lower bound per il numero di mattonelle ancora da calpestare
    int i = (u / 4) / columns;
    int j = (u / 4) % columns;

    int inc = std::min(rows - i, columns - j);
    return rows - i - inc + columns - j - inc;
}

int shortest_path(int vertices, int rows, int columns) {
        ...
        if (dist[u] > w) {
            ...
            for (auto v: adj[u]) {
                ...
                if (dist[v.first] > dist[u] + v.second) {
                    q.push({dist[u] + v.second + estimate(u, rows, columns),
                            dist[u] + v.second,
                            v.first});
                }
            }
...
int cammina(int N, int M, int griglia[][MAXN]) {
    ...
    int ans = shortest_path(4 * M * N, N, M);
